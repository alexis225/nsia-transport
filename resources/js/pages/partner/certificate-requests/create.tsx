...
export default function PartnerCertificateRequestCreate({ countries, tenant, tenants }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: t('partner.requests.create.breadcrumb'), href: '/partner/certificate-requests/create' },
    ];

    const DOCUMENT_TYPES: { key: string; label: string; hint: string }[] = [
        { key: 'BL',                  label: t('partner.requests.create.documentTypes.BL.label'),                  hint: t('partner.requests.create.documentTypes.BL.hint') },
        { key: 'FACTURE',             label: t('partner.requests.create.documentTypes.FACTURE.label'),             hint: t('partner.requests.create.documentTypes.FACTURE.hint') },
        { key: 'FDI',                 label: t('partner.requests.create.documentTypes.FDI.label'),                 hint: t('partner.requests.create.documentTypes.FDI.hint') },
        { key: 'DOCUMENTS_TRANSPORT', label: t('partner.requests.create.documentTypes.DOCUMENTS_TRANSPORT.label'), hint: t('partner.requests.create.documentTypes.DOCUMENTS_TRANSPORT.hint') },
        { key: 'AUTRE',               label: t('partner.requests.create.documentTypes.AUTRE.label'),               hint: t('partner.requests.create.documentTypes.AUTRE.hint') },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div style={{ padding: '24px', maxWidth: '900px', margin: '0 auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                    ...
                    <div>
                        <h1 style={{ fontSize: '20px', fontWeight: 700, color: '#0f172a', margin: 0 }}>{t('partner.requests.create.heading')}</h1>
                        <p style={{ color: '#64748b', fontSize: '13px', margin: '2px 0 0' }}>{t('partner.requests.create.subtitle')}</p>
                    </div>
                </div>
                ...
                <form onSubmit={e => submit(e, 'submit')}>
                    <div style={cardStyle}>
                        <h2 style={{ fontSize: '15px', fontWeight: 700, color: '#0f172a', margin: '0 0 14px', display: 'flex', alignItems: 'center', gap: 6 }}>
                            <Building2 size={16} /> {t('partner.requests.create.tenant.title')}
                        </h2>
                        {tenants.length > 0 ? (
                            <div>
                                ...
                                <div style={{ fontSize: 11.5, color: '#94a3b8', marginTop: 6 }}>
                                    {t('partner.requests.create.tenant.multipleHint')}
                                </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 6, fontSize: 13, color: '#374151' }}>
                                ...
                                <span style={{ color: '#94a3b8', fontSize: 11.5 }}>{t('partner.requests.create.tenant.attachedHint')}</span>
                            </div>
                        )}
                    </div>

                    <div style={cardStyle}>
                        <h2 style={{ fontSize: '15px', fontWeight: 700, color: '#0f172a', margin: '0 0 16px' }}>{t('partner.requests.create.shipment.title')}</h2>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.shipment.insuredName')}</label>
                                <input style={{ ...inputStyle, borderColor: errors.insured_name ? '#fca5a5' : '#e2e8f0' }}
                                       value={data.insured_name} onChange={e => setData('insured_name', e.target.value)} placeholder={t('partner.requests.create.shipment.insuredNamePlaceholder')} />
                                {errors.insured_name && <p style={errorStyle}><AlertCircle size={12} />{errors.insured_name}</p>}
                            </div>
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.shipment.country')}</label>
                                <select style={{ ...inputStyle, cursor: 'pointer' }} value={data.country_code} onChange={e => setData('country_code', e.target.value)}>
                                    <option value="">{t('partner.requests.create.shipment.countryPlaceholder')}</option>
                            ...
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.shipment.from')}</label>
                                <input style={inputStyle} value={data.voyage_from} onChange={e => setData('voyage_from', e.target.value)} placeholder={t('partner.requests.create.shipment.fromPlaceholder')} />
                            </div>
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.shipment.to')}</label>
                                <input style={inputStyle} value={data.voyage_to} onChange={e => setData('voyage_to', e.target.value)} placeholder={t('partner.requests.create.shipment.toPlaceholder')} />
                            </div>
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.shipment.date')}</label>
                                <input type="date" style={inputStyle} value={data.voyage_date} onChange={e => setData('voyage_date', e.target.value)} />
                            </div>
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.shipment.transportMode')}</label>
                                <select style={{ ...inputStyle, cursor: 'pointer' }} value={data.transport_type} onChange={e => setData('transport_type', e.target.value)}>
                                    <option value="">{t('partner.requests.create.shipment.transportModeUnspecified')}</option>
                            ...
                            <div style={{ gridColumn: '1/-1' }}>
                                <label style={labelStyle}>{t('partner.requests.create.shipment.cargoDescription')}</label>
                                <textarea style={{ ...inputStyle, resize: 'vertical' }} rows={2}
                                          ...
                                          placeholder={t('partner.requests.create.shipment.cargoDescriptionPlaceholder')} />
                            </div>
                        </div>
                    </div>

                    <div style={cardStyle}>
                        <h2 style={{ fontSize: '15px', fontWeight: 700, color: '#0f172a', margin: '0 0 16px' }}>{t('partner.requests.create.value.title')}</h2>
                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '14px' }}>
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.value.estimatedValue')}</label>
                                <input type="number" style={inputStyle} value={data.estimated_value} onChange={e => setData('estimated_value', e.target.value)} placeholder={t('partner.requests.create.value.estimatedValuePlaceholder')} />
                                {errors.estimated_value && <p style={errorStyle}><AlertCircle size={12} />{errors.estimated_value}</p>}
                            </div>
                            <div>
                                <label style={labelStyle}>{t('partner.requests.create.value.currency')}</label>
                                    ...
                                    <div key={docType.key} style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 14 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: files.length > 0 ? 10 : 0 }}>
                                            ...
                                            <button type="button"
                                                ...
                                                <Upload size={13} /> {t('partner.requests.create.documents.add')}
                                            </button>
                                            <input ref={el => {
 fileInputRefs.current[docType.key] = el;
                                                ...
                                                {files.map(({ file, i }) => (
                                                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 6 }}>
                                                        ...
                                                        <div style={{ flex: 1, minWidth: 0 }}>
                                                            ...
                                                            <p style={{ margin: 0, fontSize: 11, color: '#64748b' }}>{t('partner.requests.create.documents.sizeKo', { size: (file.size / 1024).toFixed(0) })}</p>
                                                        </div>
                    ...
                    <div style={cardStyle}>
                        <h2 style={{ fontSize: '15px', fontWeight: 700, color: '#0f172a', margin: '0 0 12px' }}>{t('partner.requests.create.notes.title')}</h2>
                        <textarea style={{ ...inputStyle, resize: 'vertical' }} rows={3}
                                  value={data.notes} onChange={e => setData('notes', e.target.value)} placeholder={t('partner.requests.create.notes.placeholder')} />
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                        ...
                        <Button type="button" variant="outline" disabled={processing} onClick={e => submit(e, 'draft')}>
                            {processing ? t('partner.requests.create.sending') : t('partner.requests.create.saveDraft')}
                        </Button>
                        <Button type="submit" disabled={processing} className="bg-[#1e3a8a] hover:bg-[#1e40af] text-white" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <Upload size={15} />
                            {processing ? t('partner.requests.create.sending') : t('partner.requests.create.submit')}
                        </Button>
                    </div>
                </form>
            </div>
