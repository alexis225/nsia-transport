...
export default function PartnerCertificateRequestShow({
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            title:
                cr.insured_name ??
                t('partner.requests.show.defaultInsuredName'),
            href: `/partner/certificate-requests/${cr.id}`,
        },
    ];
    ...
    function cancel() {
        ...
        router.delete(
            route('partner.certificate-requests.destroy', {
    ...
    function addCompletionFiles(files: FileList | null) {
        ...
        completeForm.setData({
            ...
            document_types: [
                ...completeForm.data.document_types,
                ...Array.from(files).map(() => 'AUTRE'),
            ],
        });
    }
    ...
    function removeCompletionFile(index: number) {
        completeForm.setData({
            ...completeForm.data,
            documents: completeForm.data.documents.filter(
                (_, i) => i !== index,
            ),
            document_types: completeForm.data.document_types.filter(
                (_, i) => i !== index,
            ),
        });
    }
    ...
    function submitCompletion(e: React.FormEvent) {
        e.preventDefault();
        completeForm.post(
            route('partner.certificate-requests.complete', {
                certificateRequest: cr.id,
            }),
            { forceFormData: true },
        );
    ...
    function addDraftFiles(type: string, files: FileList | null) {
        ...
        draftDocsForm.setData({
            ...
            document_types: [
                ...draftDocsForm.data.document_types,
    ...
    function removeDraftFile(index: number) {
        draftDocsForm.setData({
            ...draftDocsForm.data,
            documents: draftDocsForm.data.documents.filter(
                (_, i) => i !== index,
            ),
            document_types: draftDocsForm.data.document_types.filter(
                (_, i) => i !== index,
            ),
        });
    }
    ...
    function saveDraftDocs(e: React.FormEvent) {
        e.preventDefault();
        draftDocsForm.post(
            route('partner.certificate-requests.update', {
                certificateRequest: cr.id,
            }),
            {
                forceFormData: true,
    ...
    function submitDraft() {
        ...
        if (!confirm(t('partner.requests.show.draft.confirmSubmit'))) {
            return;
        }

        router.post(
            route('partner.certificate-requests.submit', {
                certificateRequest: cr.id,
            }),
        );
    }
    ...
    const timelineSteps = [
        {
            label: t('partner.requests.show.timeline.submitted'),
            done: !!cr.submitted_at,
            date: cr.submitted_at,
            by: null as string | null,
        },
        {
            label: t('partner.requests.show.timeline.assigned'),
            done: !!cr.assigned_at,
            date: cr.assigned_at,
            by: cr.assigned_to
            ...
            ? [
                  {
                      ...
                      by: cr.completed_at
                          ? t('partner.requests.show.timeline.completedOn', {
                                date: fmt(cr.completed_at),
                            })
                          : t('partner.requests.show.timeline.toComplete'),
                  },
              ]
            : []),
        {
            label:
                cr.status === 'REJECTED'
                    ? t('partner.requests.show.timeline.rejected')
                    : t('partner.requests.show.timeline.approved'),
            done: !!cr.reviewed_at,
            date: cr.reviewed_at,
            by: cr.reviewed_by
        ...
        },
        {
            ...
            date: null,
            by: cr.certificate
                ? t('partner.requests.show.timeline.issuedNumber', {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head
                title={t('partner.requests.show.title', {
            ...
            >
                    ...
                    >
                        ...
                        <div>
                            ...
                            >
                                {cr.insured_name ??
                                    t(
                                        'partner.requests.show.defaultInsuredName',
                                    )}
                            </h1>
                            <p
                                style={{
                            ...
                            >
                                {cr.reference ? (
                                    <>
                                        {t('partner.requests.show.reference', {
                                            reference: cr.reference,
                                        })}{' '}
                                    </>
                                ) : null}
                                {cr.status === 'DRAFT'
                                    ? t('partner.requests.show.draftSaved')
                                    : t('partner.requests.show.submittedOn', {
                ...
                <div style={cardStyle}>
                    ...
                    >
                        {t('partner.requests.show.timeline.title')}
                    </h2>
                    ...
                    >
                        ...
                        >
                            ...
                            <div>
                                ...
                                >
                                    {t('partner.requests.show.draft.title')}
                                </div>
                                <div
                                    style={{
                                        fontSize: 12.5,
                        ...
                        <form onSubmit={saveDraftDocs}>
                            {draftDocsForm.data.documents.length > 0 && (
                                ...
                                >
                                    {draftDocsForm.data.documents.map(
                                        (f, i) => (
                                            <div
                                                key={i}
                                                style={{
                            ...
                            >
                                ...
                                >
                                    <Upload size={14} />{' '}
                                    {t(
                                        'partner.requests.show.draft.addDocuments',
                                    )}
                                    <input
                                        type="file"
                                        multiple
                                ...
                                {draftDocsForm.data.documents.length > 0 && (
                                    <Button
                                        type="submit"
                                        disabled={draftDocsForm.processing}
                                        variant="outline"
                                    >
                                        {t(
                                            'partner.requests.show.draft.saveDocuments',
                                        )}
                                    </Button>
                                )}
                                <Button
                                ...
                                >
                                    <Send size={14} />{' '}
                                    {t('partner.requests.show.draft.submit')}
                                </Button>
                            </div>
                        </form>
                    </div>
                ...
                {(cr.certificate || cr.guce_certificate) && (
                    ...
                    >
                        ...
                        >
                            <Award size={16} />{' '}
                            {t('partner.requests.show.available.title')}
                        </h2>
                        <p
                            style={{
                                fontSize: 13,
                        ...
                        >
                            {t('partner.requests.show.available.number', {
                        ...
                        >
                            {cr.certificate && (
                                <>
                                    <a
                                        href={route(
                                            'partner.certificate-requests.certificate.download',
                                            { certificateRequest: cr.id },
                                        )}
                                        style={{
                                            display: 'inline-flex',
                                    ...
                                    >
                                        <Download size={13} />{' '}
                                        {t(
                                            'partner.requests.show.available.download',
                                        )}
                                    </a>
                                    {cr.certificate.qr_token && (
                                        <a
                                            href={route('certificate.verify', {
                                                token: cr.certificate.qr_token,
                                            })}
                                            target="_blank"
                                            rel="noreferrer"
                                            style={{
                                        ...
                                        >
                                            {t(
                                                'partner.requests.show.available.verify',
                                            )}
                                        </a>
                                    )}
                                </>
                            ...
                            {cr.guce_certificate && (
                                <a
                                    href={route(
                                        'partner.certificate-requests.guce-certificate.download',
                                        { certificateRequest: cr.id },
                                    )}
                                    style={{
                                        display: 'inline-flex',
                    ...
                    >
                        ...
                        >
                            ...
                            <div>
                                ...
                                >
                                    {t(
                                        'partner.requests.show.infoRequested.title',
                                    )}
                                </div>
                                <div
                                    style={{
                        ...
                        <form onSubmit={submitCompletion}>
                            <textarea
                                ...
                                placeholder={t(
                                    'partner.requests.show.infoRequested.notesPlaceholder',
                                )}
                                rows={2}
                                style={{
                                    width: '100%',
                            ...
                            >
                                ...
                                >
                                    <Upload size={14} />{' '}
                                    {t(
                                        'partner.requests.show.infoRequested.addDocuments',
                                    )}
                                    <input
                                        type="file"
                                        multiple
                                ...
                                >
                                    {t(
                                        'partner.requests.show.infoRequested.submit',
                                    )}
                                </Button>
                            </div>
                        </form>
                    ...
                    >
                        ...
                        >
                            ...
                            <div>
                                ...
                                >
                                    {t('partner.requests.show.rejected.title')}
                                </div>
                                <div
                                    style={{
                                        fontSize: 12.5,
                ...
                <div style={cardStyle}>
                    ...
                    >
                        {t('partner.requests.show.shipment.title')}
                    </h2>
                    <div
                        style={{
                            display: 'grid',
                    ...
                    >
                        <div>
                            <span style={labelStyle}>
                                {t('partner.requests.show.shipment.country')}
                            </span>
                        ...
                        <div>
                            <span style={labelStyle}>
                                {t(
                                    'partner.requests.show.shipment.transportMode',
                                )}
                            </span>
                            <span style={valueStyle}>
                                {cr.transport_type
                        ...
                        <div>
                            <span style={labelStyle}>
                                {t('partner.requests.show.shipment.route')}
                            </span>
                            <span style={valueStyle}>
                        ...
                        <div>
                            <span style={labelStyle}>
                                {t('partner.requests.show.shipment.date')}
                            </span>
                            <span style={valueStyle}>
                        ...
                        <div style={{ gridColumn: '1/-1' }}>
                            <span style={labelStyle}>
                                {t('partner.requests.show.shipment.cargo')}
                            </span>
                            <span style={valueStyle}>
                        ...
                        <div>
                            <span style={labelStyle}>
                                {t(
                                    'partner.requests.show.shipment.estimatedValue',
                                )}
                            </span>
                            <span style={valueStyle}>
                                {cr.estimated_value
                    ...
                    {cr.notes && (
                        <div style={{ marginTop: 16 }}>
                            <span style={labelStyle}>
                                {t('partner.requests.show.shipment.notes')}
                            </span>
                ...
                <div style={cardStyle}>
                    ...
                    >
                        {t('partner.requests.show.documents.title')}
                    </h2>
                    {cr.documents.length === 0 ? (
                        ...
                        >
                            {t('partner.requests.show.documents.empty')}
                        </p>
                    ) : (
                        <div
                            style={{
                                ...
                                >
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        {doc.file_size && (
                                            ...
                                            >
                                                {t(
                                                    'partner.requests.show.documents.sizeKo',
                                                    {
                                                        size: (
                                                            doc.file_size / 1024
                                                        ).toFixed(0),
                                    ...
                                    <a
                                        href={route(
                                            'partner.certificate-requests.documents.download',
                                            {
                                                certificateRequest: cr.id,
                                                document: doc.id,
                                            },
                ...
                >
                    ...
                    >
                        <ChevronLeft size={14} />{' '}
                        {t('partner.requests.show.backToList')}
                    </Link>
                    {(cr.status === 'DRAFT' || cr.status === 'PENDING') && (
                        ...
                        >
                            <Trash2 size={14} />{' '}
                            {cr.status === 'DRAFT'
                                ? t('partner.requests.show.cancel.deleteDraft')
                                : t(
                                      'partner.requests.show.cancel.cancelRequest',
                                  )}
                        </Button>
                    )}
                </div>
