...
export default function CommissionBordereau({
    transactions,
    totals,
    brokers,
    filters,
                ...
                <div className="bd-page">
                    ...
                    <div className="kpi-grid">
                        <div className="kpi-card">
                            ...
                            <div className="kpi-lbl">
                                {t('bordereau.kpis.certificates')}
                            </div>
                        </div>
                        <div className="kpi-card">
                            <div className="kpi-val" style={{ fontSize: 14 }}>
                                {totals.prime_brute.toLocaleString('fr-FR', {
                                    maximumFractionDigits: 0,
                                })}
                            </div>
                            <div className="kpi-lbl">
                                {t('bordereau.kpis.primeBrute')}
                            </div>
                        </div>
                        <div
                            className="kpi-card"
                        ...
                        >
                            ...
                            >
                                {totals.prime_nette.toLocaleString('fr-FR', {
                                    maximumFractionDigits: 0,
                                })}
                            </div>
                            <div className="kpi-lbl">
                                {t('bordereau.kpis.primeNette')}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="toolbar">
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                applyFilter({ period_from: e.target.value })
                            }
                            title={t('bordereau.filters.periodFromTitle')}
                        />
                        <span style={{ fontSize: 11, color: '#94a3b8' }}>
                            {t('bordereau.filters.to')}
                        </span>
                        <input
                            type="date"
                            className="hs-date"
                            ...
                            onChange={(e) =>
                                applyFilter({ period_to: e.target.value })
                            }
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                applyFilter({ broker_id: e.target.value })
                            }
                        >
                            <option value="">
                                {t('bordereau.filters.allBrokers')}
                            </option>
                            {brokers.map((b) => (
                        ...
                        >
                            <option value="">
                                {t('bordereau.filters.allStatuses')}
                            </option>
                            <option value="PENDING">
                                {t('bordereau.filters.statusPending')}
                            </option>
                            <option value="PAID">
                                {t('bordereau.filters.statusPaid')}
                            </option>
                            <option value="CANCELLED">
                                {t('bordereau.filters.statusCancelled')}
                            </option>
                        </select>
                        ...
                        {(filters?.broker_id || filters?.status) && (
                            <button
                                onClick={() =>
                                    router.get('/admin/commissions/bordereau')
                                }
                                style={{
                                    fontSize: 11,
                                    color: '#94a3b8',
                                ...
                                <table>
                                    <thead>
                                        <tr>
                                            <th>
                                                {t(
                                                    'bordereau.table.certificate',
                                                )}
                                            </th>
                                            <th>
                                                {t('bordereau.table.insured')}
                                            </th>
                                            <th>
                                                {t('bordereau.table.contract')}
                                            </th>
                                            <th>
                                                {t('bordereau.table.broker')}
                                            </th>
                                            <th>
                                                {t('bordereau.table.period')}
                                            </th>
                                            <th style={{ textAlign: 'right' }}>
                                                {t(
                                                    'bordereau.table.primeBrute',
                                                )}
                                            </th>
                                            <th style={{ textAlign: 'center' }}>
                                                {t('bordereau.table.rate')}
                                            </th>
                                            <th style={{ textAlign: 'right' }}>
                                                {t(
                                                    'bordereau.table.commission',
                                                )}
                                            </th>
                                            <th style={{ textAlign: 'right' }}>
                                                {t(
                                                    'bordereau.table.primeNette',
                                                )}
                                            </th>
                                            <th>
                                                {t('bordereau.table.status')}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                                ...
                                                <tr key={tx.id}>
                                                    ...
                                                    <td>
                                                        ...
                                                        >
                                                            ...
                                                            />
                                                            {t(
                                                                `bordereau.statuses.${tx.status}`,
                                                                {
                                                                    defaultValue:
                                                                        t(
                                                                            'bordereau.statuses.PENDING',
                                                                        ),
                                                                },
                                                            )}
                                                        </span>
                                    ...
                                    <tfoot>
                                        <tr className="tfoot-row">
                                            ...
                                            >
                                                {totals.prime_brute.toLocaleString(
                                                    'fr-FR',
                                                    {
                                                        minimumFractionDigits: 2,
                                                    },
                                            ...
                                            >
                                                {totals.prime_nette.toLocaleString(
                                                    'fr-FR',
                                                    {
                                                        minimumFractionDigits: 2,
                                                    },
