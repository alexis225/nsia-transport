...
export default function GuceCertificatesIndex({
    certificates,
    filters,
}: Props) {
    ...
    function handleDelete(id: string, ref: string) {
        if (!confirm(t('guce.index.confirmDelete', { reference: ref }))) {
            return;
        }
                ...
                >
                    {t(
                        certificates.total > 1
                            ? 'guce.index.count_plural'
                            : 'guce.index.count',
                        { count: certificates.total },
                    )}
                    {certificates.total > 0 &&
                        t('guce.index.countRange', {
                            ...
                            {certificates.data.length === 0 ? (
                                ...
                                certificates.data.map((cert, i) => (
                                    ...
                                    >
                                        ...
                                        >
                                            {cert.imported_by
                                                ? `${cert.imported_by.first_name} ${cert.imported_by.last_name}`
                                                : '—'}
                                        </td>
                                        <td style={{ padding: '10px 14px' }}>
                                            <div
                                            ...
                                            >
                                                <Link
                                                    href={`/admin/guce-certificates/${cert.id}`}
                                                >
                                                    <button
                                                        title={tc(
                                                            'actions.view',
