    ...
    return (
        ...
        >
            ...
            >
                {grant.granted_at && (
                    <span>
                        {t('index.card.grantedOn', {
                            date: fmt(grant.granted_at),
                        })}
                    </span>
                )}
                {grant.expires_at && (
                    ...
                    >
                        <Clock size={10} />{' '}
                        {t('index.card.expiresOn', {
                            date: fmt(grant.expires_at),
                        })}
                    </span>
                )}
                {grant.reason && (
            ...
            {grant.status === 'REVOKED' && grant.revoked_by && (
                ...
                >
                    <UserX size={11} />{' '}
                    {t('index.card.revokedBy', { name: grant.revoked_by.name })}
                    {grant.revoked_at &&
                        t('index.card.revokedOn', {
                            date: fmt(grant.revoked_at),
                        })}
                </div>
            )}
            ...
            >
                ...
                >
                    <div style={{ fontSize: 12, color: '#475569' }}>
                        {t('index.revokeModal.descriptionPrefix')}{' '}
                        <strong>{grant.role_label}</strong>{' '}
                        {t('index.revokeModal.descriptionTo')}{' '}
                        <strong>{grant.grantee?.name}</strong>
                        {grant.expires_at &&
                            t('index.revokeModal.descriptionExpiry', {
                                date: fmt(grant.expires_at),
                            })}
                        .
                    </div>
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder={t(
                                'index.revokeModal.reasonPlaceholder',
                            )}
                        />
                    </div>
                    <div
...
export default function DelegationsIndex({
    granted,
    received,
    colleagues,
    delegatableRoles,
                        ...
                        >
                            ...
                            {received.map((g) => (
                                ...
                                >
                                    <Shield size={10} />
                                    <span>
                                        {t('index.receivedBanner.role', {
                                            role: g.role_label,
                                        })}
                                    </span>
                                    <span>
                                        ·{' '}
                                        {t('index.receivedBanner.from', {
                                            name: g.grantor?.name,
                                        })}
                                    </span>
                                    {g.expires_at && (
                                        <span>
                                            ·{' '}
                                            {t('index.receivedBanner.until', {
                                                date: fmt(g.expires_at),
                                            })}
                                        </span>
                                    )}
                                </div>
                    ...
                    <div className="dl-grid mt-4">
                        <div className="dl-section">
                            <div className="dl-section-hdr">
                                <div
                                    className="dl-section-ico"
                                    style={{ background: '#f0fdf4' }}
                                >
                                    <UserCheck size={14} color="#15803d" />
                                </div>
                                <span className="dl-section-ttl">
                                    {t('index.sections.active', {
                        ...
                        <div className="dl-section">
                            <div className="dl-section-hdr">
                                ...
                                <span className="dl-section-ttl">
                                    {t('index.sections.history', {
                                        count: historyGranted.length,
                                    })}
                                </span>
                            </div>
                            <div className="dl-section-body">
