...
export default function CertificateTemplateEdit({
    ...
    const handleLogoRemove = () => {
        if (!confirm(t('edit.confirmRemoveLogo'))) {
            return;
        }

        router.delete(
            route('admin.certificate-templates.logo.remove', {
                certificateTemplate: template.id,
            }),
            {
                onSuccess: () => {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(
            route('admin.certificate-templates.update', {
                certificateTemplate: template.id,
            }),
        );
    };
                        ...
                        {logoFile ? (
                            ...
                            >
                                ...
                                <Button
                                    ...
                                    disabled={uploading}
                                    size="sm"
                                    className="h-8 bg-[#1e3a8a] px-3 text-xs text-white hover:bg-[#1e40af]"
                                >
                                    {uploading ? (
                                        t('edit.logoCard.uploading')
                                    ) : (
                                        <>
                                            <Check size={12} />{' '}
                                            {tc('actions.save')}
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('edit.headTitle', { name: template.name })} />
            <TemplateForm
                data={data}
                setData={setData}
                errors={errors}
                ...
                heroTitle={t('edit.heroTitle', { name: template.name })}
