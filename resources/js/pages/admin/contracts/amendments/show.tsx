                ...
                >
                    {requireReason && (
                        <div>
                            ...
                            >
                                {t('amendments.show.modals.reasonLabel')}
                            </label>
                            <textarea
                                value={reason}
                    ...
                    >
                        <Button variant="outline" onClick={onClose}>
                            {t('amendments.show.modals.cancel')}
                        </Button>
                        <Button
...
const formatValue = (field: string, value: any): string => {
    ...
    if (field === 'accessories_amount') {
        return parseFloat(value).toLocaleString('fr-FR');
    }

    if (field.includes('rate') || field === 'premium_rate') {
        return `${value} %`;
    }

    if (field === 'subscription_limit') {
        return parseFloat(value).toLocaleString('fr-FR');
    }
...
export default function AmendmentShow({ contract, amendment, can }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head
                title={t('amendments.show.title', {
                    number: amendment.amendment_number,
                })}
            />
            <style>{`
                ...
                <div className="as-wrap">
                    ...
                    <div className="as-hero">
                        ...
                        >
                            ...
                            <div className="as-hero-num">
                                {amendment.amendment_number}
                            </div>
                            <div className="as-hero-sub">
                                {t('amendments.show.sequenceInfo', {
                            ...
                            >
                                ...
                                >
                                    {t('amendments.show.changesBadge', {
                                        count: changesCount,
                                    })}
                                </span>
                            </div>
                    ...
                    <div className="workflow-bar">
                        ...
                        >
                            ...
                            {amendment.applied_at && (
                                ...
                                >
                                    {t('amendments.show.appliedOn', {
                                        date: fmtDt(amendment.applied_at),
                                    })}
                                </span>
                            )}
                        </div>
                        <div style={{ display: 'flex', gap: 8 }}>
                            {amendment.status === 'DRAFT' && can.edit && (
                                <button
                                    ...
                                    onClick={() =>
                                        action(
                                            'admin.contracts.amendments.submit',
                                        )
                                    }
                                >
                                    <Send size={12} />{' '}
                                    {t('amendments.show.submitForValidation')}
                                </button>
                            )}
                            {amendment.status === 'PENDING' && can.validate && (
                                <>
                                    ...
                                    >
                                        <CheckCircle size={12} />{' '}
                                        {t('amendments.show.approveApply')}
                                    </button>
                                    <button
                    ...
                    {amendment.review_notes && (
                        <div
                            style={{
                                background: amendment.review_notes.startsWith(
                                    'REJETÉ',
                                )
                                    ? '#fef2f2'
                                    : '#f0fdf4',
                                ...
                                padding: '10px 14px',
                                fontSize: 12,
                                color: amendment.review_notes.startsWith(
                                    'REJETÉ',
                                )
                                    ? '#dc2626'
                                    : '#15803d',
                        ...
                        >
                            ...
                            >
                                <AlertCircle size={13} />
                                {amendment.review_notes.startsWith('REJETÉ')
                                    ? t('amendments.show.rejectionReason')
                                    : t('amendments.show.approvalNotes')}
                            </div>
                            {amendment.review_notes}
                        </div>
                    )}
                    ...
                    <div className="as-card">
                        <div className="as-card-hdr">
                            <div
                                className="as-card-ico"
                                style={{ background: '#eff6ff' }}
                            >
                                <FileText size={15} color="#3b82f6" />
                            </div>
                            <span className="as-card-ttl">
                                {t('amendments.show.reasonSection.title')}
                            </span>
                        </div>
                        <div className="as-card-body">
                            <div
                                style={{
                                    ...
                                    fontWeight: 500,
                                    color: '#1e293b',
                                    marginBottom: amendment.description ? 8 : 0,
                                }}
                            >
                                {amendment.reason}
                            </div>
                    ...
                    <div className="as-card">
                        <div className="as-card-hdr">
                            ...
                            <span className="as-card-ttl">
                                {t('amendments.show.changesSection.title', {
                                    count: changesCount,
                                })}
                            </span>
                        </div>
                            ...
                            <table className="changes-table">
                                <thead>
                                    <tr>
                                        <th>
                                            {t(
                                                'amendments.show.changesSection.field',
                                            )}
                                        </th>
                                        <th>
                                            {t(
                                                'amendments.show.changesSection.before',
                                            )}
                                        </th>
                                        <th></th>
                                        <th>
                                            {t(
                                                'amendments.show.changesSection.after',
                                            )}
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {Object.entries(amendment.changes).map(
                                        ([field, change]) => (
                                            <tr key={field}>
                                                <td
                                                    style={{
                    ...
                    <div className="as-card">
                        <div className="as-card-hdr">
                            ...
                            <span className="as-card-ttl">
                                {t('amendments.show.history.title')}
                            </span>
                        </div>
                        <div className="as-card-body">
                            {[
                                {
                                    ...
                                    user: amendment.created_by,
                                    label: t('amendments.show.history.created'),
                                    color: '#94a3b8',
                                },
                                {
                                    date: amendment.submitted_at,
                                    user: amendment.submitted_by,
                                    label: t(
                                        'amendments.show.history.submitted',
                                    ),
                                    color: '#f59e0b',
                                },
                                {
                                    date: amendment.reviewed_at,
                                    user: amendment.reviewed_by,
                                    label:
                                        amendment.status === 'APPROVED'
                                            ? t(
                                                  'amendments.show.history.approved',
                                              )
                                            : t(
                                                  'amendments.show.history.rejected',
                                              ),
                                    color:
                                        amendment.status === 'APPROVED'
                                            ? '#16a34a'
            ...
            {modal === 'approve' && (
                <ActionModal
                    title={t('amendments.show.modals.approveTitle')}
                    icon={CheckCircle}
                    color="#15803d"
                    actionLabel={t('amendments.show.modals.approveAction')}
                    requireReason={false}
                    onConfirm={(notes: string) =>
                        action('admin.contracts.amendments.approve', { notes })
                    }
            ...
            {modal === 'reject' && (
                <ActionModal
                    title={t('amendments.show.modals.rejectTitle')}
                    icon={XCircle}
                    color="#dc2626"
                    actionLabel={t('amendments.show.modals.rejectAction')}
                    onConfirm={(reason: string) =>
                        action('admin.contracts.amendments.reject', { reason })
                    }
                    onClose={() => setModal(null)}
                />
            )}
