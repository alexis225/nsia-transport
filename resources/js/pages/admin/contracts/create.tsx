        ...
        >
            <div
                style={{
                    ...
                    height: 20,
                    borderRadius: 10,
                    background: data[field] ? '#1e3a8a' : '#e2e8f0',
                    position: 'relative',
...
function TagList({ data, setData, field, input, setInput, placeholder }: any) {
    ...
    const removeFromList = (index: number) => {
        setData(
            field,
            (data[field] ?? []).filter((_: any, i: number) => i !== index),
        );
    };
...
export function ContractForm({
    ...
    const selectedBroker = brokers?.find(
        (b: Broker) => b.id === data.broker_id,
    );

    // Taux prime global = simple somme des taux R.O. + R.G. (les seuls
    // saisis au niveau du contrat) — jamais saisi directement.
    const premiumRate =
        (parseFloat(data.rate_ro) || 0) + (parseFloat(data.rate_rg) || 0);

    const contractCoinsurers: { coinsurer_id: string; share_rate: string }[] =
        data.coinsurers ?? [];
    const selectedCoinsurerIds = new Set(
        contractCoinsurers.map((c) => c.coinsurer_id),
    );
    const availableCoinsurers = (coinsurers ?? []).filter(
        (c: CoinsurerOption) => !selectedCoinsurerIds.has(c.id),
    );
    const totalShareRate = contractCoinsurers.reduce(
        (sum, c) => sum + (parseFloat(c.share_rate) || 0),
        0,
    );

    const addCoinsurerRow = () => {
        if (!availableCoinsurers.length) {
            return;
        }

        setData('coinsurers', [
            ...contractCoinsurers,
            { coinsurer_id: availableCoinsurers[0].id, share_rate: '' },
        ]);
    };
    const updateCoinsurerRow = (
        i: number,
        field: 'coinsurer_id' | 'share_rate',
        value: string,
    ) => {
    ...
    const removeCoinsurerRow = (i: number) => {
        setData(
            'coinsurers',
            contractCoinsurers.filter((_, idx) => idx !== i),
        );
    };
                ...
                <div className="cf-wrap">
                    ...
                    >
                            ...
                            <div className="cf-card-body">
                                    ...
                                    {tenants?.length > 0 && (
                                        <div className="grid gap-2">
                                            ...
                                            >
                                                <option value="">
                                                    {t(
                                                        'form.identification.tenantPlaceholder',
                                                    )}
                                                </option>
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        >
                                            <option value="OPEN_POLICY">
                                                {t(
                                                    'form.identification.typeOpenPolicy',
                                                )}
                                            </option>
                                            <option value="VOYAGE">
                                                {t(
                                                    'form.identification.typeVoyage',
                                                )}
                                            </option>
                                            <option value="ANNUAL_VOYAGE">
                                                {t(
                                                    'form.identification.typeAnnualVoyage',
                                                )}
                                            </option>
                                            <option value="TIERS_CHARGEUR">
                                                {t(
                                                    'form.identification.typeTiersChargeur',
                                                )}
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div className="form-grid">
                                    ...
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t(
                                                'form.identification.subscriber',
                                            )}
                                        </Label>
                                        <select
                                            className="cf-select"
                                        ...
                                        >
                                            <option value="">
                                                {t(
                                                    'form.identification.noSubscriber',
                                                )}
                                            </option>
                                            {subscribers?.map(
                                                (s: Subscriber) => (
                                        ...
                                        >
                                            {t(
                                                'form.identification.subscriberHint',
                                            )}
                                        </p>
                                    </div>
                                </div>
                                {data.broker_id && (
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t(
                                                'form.identification.commissionRate',
                                            )}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            ...
                                            placeholder={t(
                                                'form.identification.commissionRatePlaceholder',
                                            )}
                                        />
                                        <InputError
                                        ...
                                        >
                                            {t(
                                                'form.identification.commissionRateHint',
                                                {
                                                    brokerName:
                                                        selectedBroker?.name ??
                                                        t(
                                                            'form.identification.defaultBrokerName',
                                                        ),
                                                    rate:
                                                        selectedBroker?.commission_rate ??
                                                        '—',
                                                },
                                            )}
                                        </p>
                        ...
                        <div className="cf-card">
                            <div className="cf-card-hdr">
                                <div className="cf-card-ttl">
                                    {t('form.coinsurers.title')}
                                </div>
                                <div className="cf-card-sub">
                                    {t('form.coinsurers.subtitle')}
                                </div>
                            </div>
                            <div className="cf-card-body">
                                    ...
                                    >
                                        {contractCoinsurers.map((row, i) => {
                                            const rowOptions = (
                                                ...
                                                (c: CoinsurerOption) =>
                                                    c.id === row.coinsurer_id ||
                                                    !selectedCoinsurerIds.has(
                                                        c.id,
                                                    ),
                                            );
                                                ...
                                                >
                                                    ...
                                                    <div style={{ flex: 1 }}>
                                                        <Input
                                                            ...
                                                            placeholder={t(
                                                                'form.coinsurers.sharePlaceholder',
                                                            )}
                                                        />
                                                        <InputError
                                                            message={
                                                                errors[
                                                                    `coinsurers.${i}.share_rate`
                                                                ]
                                                            }
                                                        />
                                                    </div>
                                        ...
                                        >
                                            {t('form.coinsurers.totalShare', {
                                                total: totalShareRate.toFixed(
                                                    2,
                                                ),
                                            })}
                                            {totalShareRate > 100 &&
                                                t('form.coinsurers.exceeds100')}
                                        </p>
                                    </div>
                                )}
                                <button
                                    type="button"
                                    onClick={addCoinsurerRow}
                                    disabled={!availableCoinsurers.length}
                                    style={{
                                        padding: '8px 14px',
                                        background: '#f8fafc',
                                        border: '1.5px dashed #cbd5e1',
                                        borderRadius: 8,
                                        cursor: availableCoinsurers.length
                                            ? 'pointer'
                                            : 'not-allowed',
                                        opacity: availableCoinsurers.length
                                            ? 1
                                            : 0.5,
                                        fontSize: 12,
                                        color: '#475569',
                                        ...
                                        gap: 5,
                                        fontFamily: 'inherit',
                                        marginTop: contractCoinsurers.length
                                            ? 10
                                            : 0,
                                    }}
                                >
                                    <Plus size={12} />{' '}
                                    {t('form.coinsurers.add')}
                                </button>
                                {!coinsurers?.length && (
                                    <p
                                        style={{
                        ...
                        <div className="cf-card">
                            <div className="cf-card-hdr">
                                <div className="cf-card-ttl">
                                    {t('form.experts.title')}
                                </div>
                                <div className="cf-card-sub">
                                    {t('form.experts.subtitle')}
                                </div>
                            </div>
                            <div className="cf-card-body">
                                {(experts ?? []).length === 0 ? (
                        ...
                        <div className="cf-card">
                            <div className="cf-card-hdr">
                                <div className="cf-card-ttl">
                                    {t('form.subscriber.title')}
                                </div>
                                <div className="cf-card-sub">
                                    {t('form.subscriber.subtitle')}
                                </div>
                            </div>
                            <div className="cf-card-body">
                                <div className="grid gap-2">
                                    <Label className="cf-label">
                                        {t('form.subscriber.name')}
                                    </Label>
                                    <Input
                                        className="h-11"
                                        ...
                                        placeholder={t(
                                            'form.subscriber.namePlaceholder',
                                        )}
                                    />
                                    <InputError
                                        message={errors.subscriber_name}
                                    />
                                </div>
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t('form.subscriber.email')}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            type="email"
                                        ...
                                        />
                                        <InputError
                                            message={errors.subscriber_email}
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t('form.subscriber.phone')}
                                        </Label>
                                        <Input
                                            className="h-11"
                                        ...
                                        />
                                        <InputError
                                            message={errors.subscriber_phone}
                                        />
                                    </div>
                                </div>
                                <div className="grid gap-2">
                                    <Label className="cf-label">
                                        {t('form.subscriber.address')}
                                    </Label>
                                    <Input
                                        className="h-11"
                                        value={data.subscriber_address ?? ''}
                                        onChange={(e) =>
                                            setData(
                                                'subscriber_address',
                                                e.target.value,
                                    ...
                                    />
                                    <InputError
                                        message={errors.subscriber_address}
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="cf-card">
                            ...
                            <div className="cf-card-body">
                                ...
                                <div className="form-grid">
                                    ...
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t('form.insured.phone')}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            value={data.insured_phone ?? ''}
                                            onChange={(e) =>
                                                setData(
                                                    'insured_phone',
                                                    e.target.value,
                            ...
                            <div className="cf-card-body">
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        ...
                                        >
                                            <option value="">
                                                {t('form.coverage.unspecified')}
                                            </option>
                                            <option value="TOUS_RISQUES">
                                                {t('form.coverage.allRisks')}
                                            </option>
                                            <option value="FAP_SAUF">
                                                {t('form.coverage.fapSauf')}
                                            </option>
                                            <option value="FAP_ABSOLUE">
                                                {t('form.coverage.fapAbsolue')}
                                            </option>
                                        </select>
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t('form.coverage.incoterm')}
                                        </Label>
                                        <select
                                            className="cf-select"
                                            value={data.incoterm_code ?? ''}
                                            onChange={(e) =>
                                                setData(
                                                    'incoterm_code',
                                                    e.target.value,
                                        ...
                                        >
                                            <option value="">
                                                {t(
                                                    'form.coverage.allIncoterms',
                                                )}
                                            </option>
                                            {incoterms?.map((i: Incoterm) => (
                                                <option
                                                    key={i.code}
                                                    value={i.code}
                                                >
                                ...
                                <div className="form-grid">
                                    ...
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t(
                                                'form.coverage.conditioningType',
                                            )}
                                        </Label>
                                        <div
                                            style={{
                                        ...
                                        >
                                            {t(
                                                'form.coverage.conditioningHint',
                                            )}
                                        </p>
                                    </div>
                                </div>
                                <div className="grid gap-2">
                                    ...
                                    <TagList
                                        ...
                                        field="clauses"
                                        input={clauseInput}
                                        setInput={setClauseInput}
                                        placeholder={t(
                                            'form.coverage.clausesPlaceholder',
                                        )}
                                    />
                                </div>
                                <div className="grid gap-2">
                                    <Label className="cf-label">
                                        {t('form.coverage.exclusions')}
                                    </Label>
                                    <TagList
                                        data={data}
                                        setData={setData}
                                        ...
                                        placeholder={t(
                                            'form.coverage.exclusionsPlaceholder',
                                        )}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="cf-card-body">
                                ...
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t(
                                                'form.financial.globalPremiumRate',
                                            )}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            value={premiumRate.toFixed(4)}
                                            readOnly
                                            disabled
                                            style={{
                                                background: '#f8fafc',
                                        ...
                                        >
                                            {t(
                                                'form.financial.globalPremiumRateHint',
                                            )}
                                        </p>
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t('form.financial.deductible')}
                                        </Label>
                                        <AmountInput
                                            className="h-11"
                                            value={data.deductible ?? '0'}
                                            onChange={(v) =>
                                                setData('deductible', v)
                                            }
                                        />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    >
                                        {t('form.financial.detailedRates')}
                                    </Label>
                                ...
                                <div className="grid gap-2">
                                    <Label className="cf-label">
                                        {t('form.financial.accessories')}
                                    </Label>
                                    <AmountInput
                                        className="h-11"
                                        value={data.accessories_amount ?? ''}
                                        onChange={(v) =>
                                            setData('accessories_amount', v)
                                        }
                                        placeholder={t(
                                            'form.financial.accessoriesPlaceholder',
                                        )}
                                    />
                                    <InputError
                                        message={errors.accessories_amount}
                                    />
                                    <p
                                        style={{
                                            fontSize: 11,
                                    ...
                                    >
                                        {t('form.financial.accessoriesHint', {
                                            currency:
                                                data.currency_code ||
                                                t(
                                                    'form.financial.defaultCurrency',
                                                ),
                                        })}
                                    </p>
                                </div>
                                <div className="grid gap-2">
                                    <Label className="cf-label">
                                        {t('form.financial.certificatesLimit')}
                                    </Label>
                                    <Input
                                        className="h-11"
                                        type="number"
                                        min={1}
                                        value={data.certificates_limit ?? ''}
                                        onChange={(e) =>
                                            setData(
                                                'certificates_limit',
                                                e.target.value,
                                        ...
                                        placeholder={t(
                                            'form.financial.certificatesLimitPlaceholder',
                                        )}
                                    />
                                </div>
                            </div>
                        ...
                        <div className="cf-card">
                            <div className="cf-card-hdr">
                                <div className="cf-card-ttl">
                                    {t('form.escalade.title')}
                                </div>
                                <div className="cf-card-sub">
                                    {t('form.escalade.subtitle')}
                                </div>
                            </div>
                            <div className="cf-card-body">
                                <div className="grid gap-2">
                                    <Label className="cf-label">
                                        {t('form.escalade.plein')}
                                    </Label>
                                    <AmountInput
                                        className="h-11"
                                        value={data.plein ?? ''}
                                        onChange={(v) => setData('plein', v)}
                                        placeholder={t(
                                            'form.escalade.pleinPlaceholder',
                                        )}
                                    />
                                ...
                                <Toggle
                                    data={data}
                                    setData={setData}
                                    field="escalade_enabled"
                                    label={t('form.escalade.enabledLabel')}
                                    hint={t('form.escalade.enabledHint')}
                                />
                                {data.escalade_enabled && (
                                    <div className="grid gap-2">
                                        <Label className="cf-label">
                                            {t('form.escalade.threshold')}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            type="number"
                                            ...
                                            placeholder={t(
                                                'form.escalade.thresholdPlaceholder',
                                            )}
                                        />
                                        <InputError
                                            message={
                                                errors.escalade_threshold_pct
                                            }
                                        />
                                    </div>
                                )}
                        ...
                        <div className="cf-card">
                            ...
                            <div className="cf-card-body">
                                <Toggle
                                    ...
                                    label={t(
                                        'form.options.requiresApprovalLabel',
                                    )}
                                    hint={t(
                                        'form.options.requiresApprovalHint',
                                    )}
                                />
                                <div className="grid gap-2">
                                    <Label className="cf-label">
                                        {t('form.options.internalNotes')}
                                    </Label>
                                    <textarea
                                        className="cf-textarea"
                                        rows={3}
                                        ...
                                        placeholder={t(
                                            'form.options.internalNotesPlaceholder',
                                        )}
                                    />
                                </div>
                            </div>
