...
export default function ApprovalConfigs({
    ...
    const TRIGGER_LABELS: Record<TriggerType, string> = {
        insured_value_pct_of_contract: t(
            'configs.triggers.insured_value_pct_of_contract',
        ),
        subscription_limit_exceeded: t(
            'configs.triggers.subscription_limit_exceeded',
        ),
        certificates_limit_reached: t(
            'configs.triggers.certificates_limit_reached',
        ),
    };
    ...
    const startEdit = (c: Config) => {
        ...
        setData({
            ...
            threshold_pct:
                c.threshold_pct != null ? String(c.threshold_pct) : '15',
            steps:
                c.steps.length > 0
                    ? c.steps.map((s) => ({
                          role: s.role,
    ...
    const handleSubmit = () => {
        ...
        if (editingId) {
            patch(
                route('admin.approvals.configs.update', { config: editingId }),
                { onSuccess },
            );
        } else {
    ...
    const toggle = (id: string) =>
        router.patch(route('admin.approvals.configs.toggle', { config: id }));
    const destroy = (id: string) => {
        if (confirm(t('configs.deleteConfirm'))) {
            router.delete(
                route('admin.approvals.configs.destroy', { config: id }),
            );
        }
    };
                ...
                <div className="ac-page">
                    ...
                    >
                        ...
                        {!showForm && (
                            <Button
                                onClick={startCreate}
                                className="h-10 bg-[#7c1f1f] px-4 text-white hover:bg-[#991b1b]"
                            >
                        ...
                        <div className="form-card">
                            <div className="form-card-hdr">
                                <span className="form-card-ttl">
                                    {editingId
                                        ? t('configs.form.editTitle')
                                        : t('configs.form.createTitle')}
                                </span>
                                <button
                                    onClick={cancelForm}
                                    style={{
                            ...
                            <div className="form-card-body">
                                {isSA && tenants?.length > 0 && (
                                    <div className="grid gap-2">
                                        ...
                                        >
                                            <option value="">
                                                {t(
                                                    'configs.form.tenantPlaceholder',
                                                )}
                                            </option>
                                            {tenants.map((tn) => (
                                                <option
                                    ...
                                    {data.trigger_type ===
                                        ...
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                placeholder={t(
                                                    'configs.form.thresholdPlaceholder',
                                                )}
                                            />
                                ...
                                >
                                    {data.trigger_type ===
                                        'insured_value_pct_of_contract' &&
                                        t(
                                            'configs.form.triggerHelp.insured_value_pct_of_contract',
                                        )}
                                    {data.trigger_type ===
                                        'subscription_limit_exceeded' &&
                                        t(
                                            'configs.form.triggerHelp.subscription_limit_exceeded',
                                        )}
                                    {data.trigger_type ===
                                        'certificates_limit_reached' &&
                                        t(
                                            'configs.form.triggerHelp.certificates_limit_reached',
                                        )}
                                </div>
                                        ...
                                        <div key={i} className="step-row">
                                            <div className="grid gap-1">
                                                ...
                                                <select
                                                    ...
                                                    className="hs-select"
                                                >
                                                    <option value="admin_filiale">
                                                        {t(
                                                            'configs.roles.admin_filiale',
                                                        )}
                                                    </option>
                                                    <option value="super_admin">
                                                        {t(
                                                            'configs.roles.super_admin',
                                                        )}
                                                    </option>
                                                </select>
                                            </div>
                                ...
                                <div style={{ display: 'flex', gap: 8 }}>
                                    <Button
                                        ...
                                        className="h-10 bg-[#7c1f1f] px-5 text-white hover:bg-[#991b1b]"
                                    >
                                        {processing ? (
                                            tc('states.saving')
                                        ) : (
