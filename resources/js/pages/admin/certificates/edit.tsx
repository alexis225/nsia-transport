...
interface ExpeditionItemRaw {
    marks: string | null; package_count: number | null;
    weight: string | null; nature: string; packaging: string | null; insured_value: number | string;
}
interface Certificate {
    id: string; certificate_number: string; status: string;
    contract_id: string; insured_name: string; insured_ref: string | null;
    voyage_date: string; voyage_from: string; voyage_to: string; voyage_via: string | null;
    origin_country_code: string | null; destination_country_code: string | null;
    transport_type: string | null; vessel_name: string | null; flight_number: string | null;
    voyage_mode: string | null; expedition_items: ExpeditionItemRaw[];
    insured_value: string; insured_value_letters: string | null; guarantee_mode: string | null;
    exchange_currency: string | null; exchange_rate: string | null;
    rate_divers: string | null; rate_surprime: string | null;
...
export default function CertificateEdit({ certificate, contracts, countries, currencies }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        expedition_items:      (certificate.expedition_items?.length ? certificate.expedition_items : [null]).map((item): ExpeditionItem => item ? ({
            marks:         item.marks ?? '',
            package_count: item.package_count != null ? String(item.package_count) : '',
            weight:        item.weight ?? '',
            nature:        item.nature ?? '',
            packaging:     item.packaging ?? '',
            insured_value: item.insured_value != null ? String(item.insured_value) : '',
        }) : emptyItem()),
        insured_value:         certificate.insured_value ?? '',
        insured_value_letters: certificate.insured_value_letters ?? '',
        guarantee_mode:        certificate.guarantee_mode ?? COVERAGE_LABELS[certificate.contract?.coverage_type ?? ''] ?? '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.certificates.update', { certificate: certificate.id }));
    };

    const rejectionBanner = certificate.status === 'REJECTED' && certificate.rejection_reason && (
