...
export default function CertificateCreate({
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        guarantee_mode:
            COVERAGE_LABELS[selectedContract?.coverage_type ?? ''] ?? '',
        exchange_currency: '',
        exchange_rate: '',
        rate_divers: '',
        rate_surprime: '',
...
export function CertificateForm({
    ...
    const selectedC: Contract | undefined = contracts.find(
        (c: Contract) => c.id === data.contract_id,
    );
    ...
    // nouveau).
    const isVoyageLocked =
        !isEditing &&
        selectedC?.type === 'VOYAGE' &&
        selectedC.active_certificates_count > 0;

    // Recalcule la valeur totale depuis les items
    const totalValue = data.expedition_items.reduce(
        (sum: number, item: ExpeditionItem) => {
    ...
    const totalPackages = data.expedition_items.reduce(
        (sum: number, item: ExpeditionItem) => {
    ...
    useEffect(() => {
        setData(
            'insured_value_letters',
            totalValue > 0
                ? amountInWords(totalValue, selectedC?.currency_code)
                : '',
        );
    ...
    const updateItem = (
        ...
        const total = items.reduce(
            (s: number, it: ExpeditionItem) =>
                s + (parseFloat(it.insured_value) || 0),
            0,
        );
    ...
    const addItem = () =>
        setData('expedition_items', [...data.expedition_items, emptyItem()]);
    const removeItem = (i: number) =>
        setData(
            'expedition_items',
            data.expedition_items.filter(
                (_: ExpeditionItem, idx: number) => idx !== i,
            ),
        );
    ...
    async function fetchExchangeRate(fromCurrency: string) {
        if (!fromCurrency || !selectedC?.currency_code) {
            return;
        }
        ...
        try {
            const res = await fetch(
                `${route('admin.certificates.exchange-rate')}?from=${fromCurrency}&to=${selectedC.currency_code}`,
                {
            ...
            if (!res.ok || !json.success) {
                setRateStatus('error');
                setRateMessage(
                    json.message ?? t('create.financial.rateUnavailable'),
                );
    ...
    const primeRatePreview = selectedC
        ? [
              ...
              {
                  label: 'Surprime',
                  rate: parseFloat(data.rate_surprime || '0'),
              },
          ].map((l) => ({
              ...l,
              amount: l.rate > 0 ? round2((totalValue * l.rate) / 100) : 0,
          }))
        : [];

    const accessoiresPreview = round2(
        parseFloat(selectedC?.accessories_amount ?? '0'),
    );
    const primeNettePreview = round2(
        primeRatePreview.reduce((s, l) => s + l.amount, 0),
    );
            ...
            <div className="flex h-full flex-1 flex-col overflow-x-auto p-4">
                <div className="cc-wrap">
                    <div className="cc-hero">
                        <div className="cc-hero-ico">
                            <Award size={22} color="rgba(255,255,255,0.8)" />
                        </div>
                    ...
                    >
                        ...
                        <div className="cc-card">
                            <div className="cc-card-hdr">
                                <div className="cc-card-ttl">
                                    {t('create.contract.title')}
                                </div>
                                <div className="cc-card-sub">
                                    {t('create.contract.subtitle')}
                                </div>
                            </div>
                            <div className="cc-card-body">
                                <div className="grid gap-2">
                                    <Label className="cc-label">
                                        {t('create.contract.label')}
                                    </Label>
                                    <select
                                        className="cc-select"
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (c) {
                                                ...
                                                setData(
                                                    'transport_type',
                                                    c.transport_mode?.code ??
                                                        'SEA',
                                                );
                                                setData(
                                                    'voyage_mode',
                                                    c.conditioning_types?.[0] ??
                                                        '',
                                                );
                                            }
                                        }}
                                ...
                                {selectedC && (
                                    <div className="contract-info">
                                        ...
                                        <div>
                                            {t('create.contract.subsidiary', {
                                                name: selectedC.tenant?.name,
                                                currency:
                                                    selectedC.currency_code,
                                            })}
                                        </div>
                                        <div style={{ marginTop: 3 }}>
                                            {t('create.contract.insured', {
                                                name: selectedC.insured_name,
                                            })}
                                        </div>
                                        <div style={{ marginTop: 3 }}>
                                            {t('create.contract.manager', {
                                                name: selectedC.subscriber
                                                    ? `${selectedC.subscriber.first_name} ${selectedC.subscriber.last_name}`
                                                    : '—',
                                            })}
                                        </div>
                                        {selectedC.insured_address && (
                                            <div style={{ marginTop: 3 }}>
                                                {t('create.contract.address', {
                                                    address:
                                                        selectedC.insured_address,
                                                })}
                                            </div>
                                        ...
                                        <div style={{ marginTop: 3 }}>
                                            {t('create.contract.contact', {
                                                phone:
                                                    selectedC.insured_phone ??
                                                    '—',
                                                email:
                                                    selectedC.insured_email ??
                                                    '—',
                                            })}
                                        </div>
                                        <div style={{ marginTop: 3 }}>
                                            {t('create.contract.commission', {
                                                value: selectedC.broker
                                                    ? `${selectedC.broker.name} (${selectedC.broker.commission_rate ?? 0} %)`
                                                    : '—',
                                            })}
                                        </div>
                                        {selectedC.subscription_limit && (
                                            <div style={{ marginTop: 3 }}>
                                                {t('create.contract.limit', {
                                                    limit: parseFloat(
                                                        selectedC.subscription_limit,
                                        ...
                                        <div style={{ marginTop: 3 }}>
                                            {t(
                                                'create.contract.certificatesCount',
                                                {
                                                    count: selectedC.certificates_count,
                                                },
                                            )}
                                            {selectedC.certificates_limit
                                                ? ` / ${selectedC.certificates_limit}`
                                                : ''}
                                        </div>
                                        <div style={{ marginTop: 3 }}>
                                            {t('create.contract.plein', {
                                                value: selectedC.plein
                                                    ? `${parseFloat(selectedC.plein).toLocaleString('fr-FR')} ${selectedC.currency_code}`
                                                    : t(
                                                          'create.contract.pleinUndefined',
                                                      ),
                                            })}
                                        </div>
                                    </div>
                            ...
                            <div className="cc-card-body">
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        ...
                                        />
                                        <InputError
                                            message={errors.insured_name}
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t('create.insuredSection.ref')}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            value={data.insured_ref ?? ''}
                                            onChange={(e) =>
                                                setData(
                                                    'insured_ref',
                                                    e.target.value,
                                            ...
                                            placeholder={t(
                                                'create.insuredSection.refPlaceholder',
                                            )}
                                        />
                                    </div>
                                </div>
                        ...
                        <div className="cc-card">
                            <div className="cc-card-hdr">
                                <div className="cc-card-ttl">
                                    {t('create.voyage.title')}
                                </div>
                                <div className="cc-card-sub">
                                    {t('create.voyage.subtitle')}
                                </div>
                            </div>
                            <div className="cc-card-body">
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        ...
                                        />
                                        <InputError
                                            message={errors.voyage_date}
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t('create.voyage.transportMode')}
                                        </Label>
                                        <select
                                            className="cc-select"
                                        ...
                                        >
                                            ...
                                            <option value="MULTIMODAL">
                                                {t(
                                                    'shared.transport.MULTIMODAL',
                                                )}
                                            </option>
                                            <option value="RIVER">
                                                {t('shared.transport.RIVER')}
                                            </option>
                                        </select>
                                    </div>
                                </div>
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            ...
                                            placeholder={t(
                                                'create.voyage.fromPlaceholder',
                                            )}
                                        />
                                        <InputError
                                            message={errors.voyage_from}
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                        ...
                                        <Input
                                            ...
                                            placeholder={t(
                                                'create.voyage.toPlaceholder',
                                            )}
                                        />
                                        <InputError
                                            message={errors.voyage_to}
                                        />
                                    </div>
                                </div>
                                <div className="form-grid">
                                ...
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        ...
                                        <InputError
                                            message={errors.origin_country_code}
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t(
                                                'create.voyage.destinationCountry',
                                            )}
                                        </Label>
                                        <select
                                            className="cc-select"
                                        ...
                                        <InputError
                                            message={
                                                errors.destination_country_code
                                            }
                                        />
                                        <p
                                            style={{
                                        ...
                                        >
                                            {t(
                                                'create.voyage.destinationCountryHint',
                                            )}
                                        </p>
                                    </div>
                                </div>
                                <div className="form-grid">
                                    {(data.transport_type === 'SEA' ||
                                        data.transport_type === 'RIVER' ||
                                        !data.transport_type) && (
                                        <div className="grid gap-2">
                                            <Label className="cc-label">
                                                {t('create.voyage.vessel')}
                                            </Label>
                                            <Input
                                                className="h-11"
                                                ...
                                                placeholder={t(
                                                    'create.voyage.vesselPlaceholder',
                                                )}
                                            />
                                        </div>
                                    )}
                                    {data.transport_type === 'AIR' && (
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                placeholder={t(
                                                    'create.voyage.flightNumberPlaceholder',
                                                )}
                                            />
                                        </div>
                                    )}
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t('create.voyage.mode')}
                                        </Label>
                                        <select
                                            className="cc-select"
                                            value={data.voyage_mode ?? ''}
                                            onChange={(e) =>
                                                setData(
                                                    'voyage_mode',
                                                    e.target.value,
                        ...
                        <div className="cc-card">
                            <div className="cc-card-hdr">
                                <div className="cc-card-ttl">
                                    {t('create.expedition.title')}
                                </div>
                                <div className="cc-card-sub">
                                    {t('create.expedition.subtitle')}
                                </div>
                            </div>
                            <div className="cc-card-body">
                                    ...
                                    <table className="exp-table">
                                        <thead>
                                            <tr>
                                                <th>
                                                    {t(
                                                        'create.expedition.marks',
                                                    )}
                                                </th>
                                                <th>
                                                    {t(
                                                        'create.expedition.count',
                                                    )}
                                                </th>
                                                <th>
                                                    {t(
                                                        'create.expedition.weight',
                                                    )}
                                                </th>
                                                <th style={{ minWidth: 220 }}>
                                                    {t(
                                                        'create.expedition.nature',
                                                    )}
                                                </th>
                                                <th>
                                                    {t(
                                                        'create.expedition.packaging',
                                                    )}
                                                </th>
                                                <th>
                                                    {t(
                                                        'create.expedition.value',
                                                    )}
                                                </th>
                                                <th></th>
                                            </tr>
                                                    ...
                                                    <tr key={i}>
                                                        <td>
                                                            <input
                                                                ...
                                                                placeholder={t(
                                                                    'create.expedition.marksPlaceholder',
                                                                )}
                                                            />
                                                        </td>
                                                        <td
                                                        ...
                                                        >
                                                            <input
                                                                ...
                                                                placeholder={t(
                                                                    'create.expedition.countPlaceholder',
                                                                )}
                                                            />
                                                        </td>
                                                        <td
                                                        ...
                                                        >
                                                            <input
                                                                ...
                                                                placeholder={t(
                                                                    'create.expedition.weightPlaceholder',
                                                                )}
                                                            />
                                                        </td>
                                                        <td>
                                                            <textarea
                                                                ...
                                                                placeholder={t(
                                                                    'create.expedition.naturePlaceholder',
                                                                )}
                                                            />
                                                        </td>
                                                        <td>
                                                            <input
                                                                ...
                                                                placeholder={t(
                                                                    'create.expedition.packagingPlaceholder',
                                                                )}
                                                            />
                                                        </td>
                                                        <td
                                ...
                                >
                                    <Plus size={12} />{' '}
                                    {t('create.expedition.addLine')}
                                </button>
                                {errors['expedition_items'] && (
                                    <p
                                        style={{
                                            fontSize: 11,
                                            color: '#ef4444',
                                ...
                                <div className="total-bar">
                                    <span className="total-label">
                                        {t('create.expedition.totalPackages')}
                                    </span>
                                    <span className="total-value">
                                        {totalPackages.toLocaleString('fr-FR')}
                                    </span>
                                </div>
                                <div className="total-bar">
                                    <span className="total-label">
                                        {t('create.expedition.totalValue')}
                                    </span>
                                    <span className="total-value">
                                ...
                                <div className="grid gap-2">
                                    <Label className="cc-label">
                                        {t('create.expedition.valueLetters')}
                                    </Label>
                                    <Input
                                        className="h-11"
                                        readOnly
                                        value={data.insured_value_letters ?? ''}
                                        style={{
                                            background: '#f8fafc',
                                            color: '#475569',
                                            cursor: 'default',
                                        ...
                                        placeholder={t(
                                            'create.expedition.valueLettersPlaceholder',
                                        )}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="cc-card-body">
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t(
                                                'create.financial.guaranteeMode',
                                            )}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            ...
                                            value={
                                                data.guarantee_mode ||
                                                t(
                                                    'create.financial.guaranteeModeUndefined',
                                                )
                                            }
                                            style={{
                                                background: '#f8fafc',
                                        ...
                                        >
                                            {t(
                                                'create.financial.guaranteeModeHint',
                                            )}
                                        </p>
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t(
                                                'create.financial.quoteCurrency',
                                            )}
                                        </Label>
                                        <select
                                            className="cc-select"
                                            value={data.exchange_currency ?? ''}
                                            onChange={(e) => {
                                        ...
                                        >
                                            <option value="">
                                                {t(
                                                    'create.financial.quoteCurrencyNone',
                                                )}
                                            </option>
                                ...
                                {data.exchange_currency && (
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t('create.financial.rateToday', {
                                                from: data.exchange_currency,
                                                to:
                                                    selectedC?.currency_code ??
                                                    '—',
                                            })}
                                ...
                                <div className="form-grid">
                                    <div className="grid gap-2">
                                        ...
                                        />
                                        <InputError
                                            message={errors.rate_divers}
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        <Label className="cc-label">
                                            {t('create.financial.rateSurprime')}
                                        </Label>
                                        <Input
                                            className="h-11"
                                            type="number"
                                        ...
                                        >
                                            {t(
                                                'create.financial.rateSurprimeHint',
                                            )}
                                        </p>
                                    </div>
                                </div>
                                ...
                                {selectedC && (
                                    ...
                                    >
                                        <Label className="cc-label">
                                            {t(
                                                'create.financial.primePreview.label',
                                            )}
                                        </Label>
                                        <div
                                            style={{
                                        ...
                                        >
                                            ...
                                            >
                                                <span>
                                                    {t(
                                                        'create.financial.primePreview.primeNette',
                                                    )}
                                                </span>
                                                <span
                                                    style={{
                                                ...
                                                >
                                                    {primeNettePreview.toLocaleString(
                                                        'fr-FR',
                                                    )}{' '}
                                                    {selectedC.currency_code}
                                                </span>
                                            ...
                                            >
                                                <span>
                                                    {t(
                                                        'create.financial.primePreview.accessories',
                                                    )}
                                                </span>
                                                <span
                                                    style={{
                                                ...
                                                >
                                                    {accessoiresPreview.toLocaleString(
                                                        'fr-FR',
                                                    )}{' '}
                                                    {selectedC.currency_code}
                                                </span>
                                            ...
                                            >
                                                <span>
                                                    {t(
                                                        'create.financial.primePreview.subtotal',
                                                    )}
                                                </span>
                                                <span
                                                    style={{
                                            ...
                                            >
                                                {t(
                                                    'create.financial.primePreview.note',
                                                )}
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                            <Button
                                type="submit"
                                disabled={processing || isVoyageLocked}
