...
function useRefConfig(t: (key: string, options?: any) => any) {
    ...
    const TABS = [
        ...
        {
            key: 'incoterms',
            label: t('tabs.incoterms'),
            icon: Ship,
            color: '#7c3aed',
        },
        {
            key: 'transport_modes',
            label: t('tabs.transportModes'),
            icon: Truck,
            color: '#0f766e',
        },
        {
            key: 'merchandise_categories',
            label: t('tabs.merchandiseCategories'),
            icon: Package,
            color: '#f97316',
        },
    ];
    ...
    > = {
        ...
        merchandise_categories: {
            columns: t('columns.merchandise_categories', {
                returnObjects: true,
            }),
    ...
    const FORM_FIELDS: Record<
        string,
        {
            key: string;
            label: string;
            type: 'text' | 'boolean' | 'number' | 'textarea' | 'array';
        }[]
    > = {
        countries: [
...
function RefModal({
    ...
    const initialData = Object.fromEntries(
        formDefs.map((f) => [
            f.key,
            item?.[f.key] ??
                (f.type === 'boolean' ? false : f.type === 'number' ? 1 : ''),
        ]),
    );
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (isEdit) {
            put(route('admin.reference.update', { tab, id: pk }), {
                onSuccess: onClose,
            });
        } else {
            post(route('admin.reference.store', { tab }), {
                onSuccess: () => {
                    reset();
                    onClose();
                },
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            {tabInfo && (
                                <tabInfo.icon size={16} color={tabInfo.color} />
                            )}
                        </div>
                        <p
                            style={{
                        ...
                        >
                            {isEdit ? t('modal.edit') : t('modal.new')} —{' '}
                            {tabInfo?.label}
                        </p>
                    </div>
                    <button
...
export default function ReferenceIndex({ tab, data, filters, counts }: Props) {
    ...
    const handleToggle = (item: RefItem) =>
        router.patch(
            route('admin.reference.toggle', { tab, id: item[cfg.pk] }),
        );
    ...
    const renderCell = (item: RefItem, field: string) => {
        ...
        if (field === 'compatible_modes' && Array.isArray(v)) {
            return (
        ...
        if (typeof v === 'string' && v.length > 60) {
            return (
                <span title={v} style={{ fontSize: 12 }}>
                    {v.slice(0, 60)}…
                </span>
            );
        }
                ...
                <div className="ref-page">
                    ...
                    <div className="ref-tabs">
                        {TABS.map((tabItem) => (
                            <button
                                key={tabItem.key}
                                className={`ref-tab ${tab === tabItem.key ? 'active' : ''}`}
                    ...
                    <div className="ref-toolbar">
                        ...
                        >
                            <input
                                ...
                                placeholder={t('searchPlaceholder', {
                                    tab: currentTab.label.toLowerCase(),
                                })}
                            />
                            <button type="submit">
                                <Search size={14} />
                        ...
                        {filters?.search && (
                            <button
                                onClick={() =>
                                    router.get('/admin/reference', { tab })
                                }
                                style={{
                                    padding: '9px 12px',
                                    background: 'none',
                    ...
                    <div className="ref-card">
                        {data.data.length === 0 ? (
                            <div className="ref-empty">
                                {t('noEntries', { tab: currentTab.label })}
                            </div>
                        ) : (
                            <>
                                <table>
                                            ...
                                            <tr key={item[cfg.pk] ?? i}>
                                                ...
                                                <td>
                                                    <div className="actions">
                                                        ...
                                                        {cfg.canToggle && (
                                                            <button
                                                                className={`btn-act ${item.is_active ? 'btn-on' : 'btn-off'}`}
                                                                onClick={() =>
                                                                    handleToggle(
                                                                        item,
                                                                    )
