...
export default function PrintOnFormButton({
    certificateId,
    templateId,
    availableTemplates,
    offsetX = 0,
    ...
    const offsetQs =
        offsetX || offsetY ? `&offset_x=${offsetX}&offset_y=${offsetY}` : '';

    return (
        <a
            href={
                supported
                    ? route('admin.certificates.print-on-form', {
                          certificate: certificateId,
                      }) + `?template=${templateId}${offsetQs}`
                    : undefined
            }
            target="_blank"
            ...
            title={
                supported
                    ? undefined
                    : `Aucun positionnement calibré pour « ${templateId} » (voir config/certificate_layouts.php)`
            }
        ...
        >
            <Button
                disabled={!supported}
                style={{
                    background: supported ? '#1e3a8a' : undefined,
                    color: supported ? '#fff' : undefined,
                    display: 'flex',
                    alignItems: 'center',
                    gap: 6,
                }}
