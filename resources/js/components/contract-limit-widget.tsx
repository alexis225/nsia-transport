...
export default function ContractLimitWidget({
    contractId,
    initialStatus,
}: Props) {
    ...
    const refresh = async () => {
        ...
        try {
            const res = await fetch(
                `/admin/contracts/${contractId}/limit-status`,
                {
                    headers: {
            ...
            // Erreur réseau ponctuelle : on réessaiera au prochain intervalle de polling.
        }
    ...
    return (
        <div
            style={{
                background: '#fff',
                border: `1.5px solid ${status.alert_level === 'ok' ? '#e2e8f0' : al.border}`,
                borderRadius: 14,
                overflow: 'hidden',
            }}
        >
            ...
            <div style={{ padding: 20 }}>
                ...
                <div style={{ marginBottom: 16 }}>
                    ...
                    {!status.can_issue && (
                        ...
                        >
                            ...
                            Plafond atteint — émission de nouveaux certificats
                            bloquée
                        </div>
                    )}
                </div>
                ...
                >
                    {[
                        {
                            label: 'Utilisé',
                            value: fmt(status.used_limit, status.currency_code),
                            color: '#1e293b',
                        },
                        {
                            label: 'Restant',
                            value: fmt(
                                status.remaining_limit ?? 0,
                ...
                {/* Certificats */}
                {(status.certificates_limit !== null ||
                                ...
                                >
                                    ...
                                    >
                                        {cert.status === 'CANCELLED'
                                            ? '−'
                                            : '+'}
                                        {cert.insured_value.toLocaleString(
                                            'fr-FR',
                                        )}
                                    </span>
                                    <span
